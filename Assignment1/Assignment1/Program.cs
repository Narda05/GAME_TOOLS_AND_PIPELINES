...
// Assignment 1
// NAME: Narda Limon Lagunas 
// STUDENT NUMBER: 2342884
// DATE: 
        ...
        public static void Main(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if (args[i] == "-i" || args[i] == "--input")
                {
                    ...
                    if (args.Length > i + 1)
                    {
                        ...
                        else if (!File.Exists(inputFile))
                        {
                            ...
                            Console.WriteLine("The input file specified does not exist: {0}", inputFile);
                        }
                        else
                        {
                ...
                else if (args[i] == "-s" || args[i] == "--sort")
                {
                    ...
                    {
                        // default to Name if no column specified
                        sortColumnName = "Name";
                    }
            ...
            if (sortEnabled)
            {
                ...
                sortColumnName = (sortColumnName ?? "Name").Trim();
                ...
                switch (sortColumnName.ToLowerInvariant())
                {
                    case "name":
                    case "comparebyname":
                    ...
                    case "rarity":
                    case "comparebyrarity":
                        results.Sort(Weapon.CompareByRarity);
                        break;

                    case "baseattack":
                    case "base_attack":
                    case "comparebybaseattack":
                        results.Sort(Weapon.CompareByBaseAttack);
                        break;

                    default:
                        Console.WriteLine("Unknown sort column '{0}', defaulting to Name.", sortColumnName);
                        results.Sort(Weapon.CompareByName);
                        break;
                }
            }
            ...
            if (results.Count > 0)
            {
                if (!string.IsNullOrEmpty(outputFile))
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(fs))
                    {
                        ...
                        {
                            writer.WriteLine("{0},{1},{2},{3}", w.Name, w.Type, w.Rarity, w.BaseAttack);
                        }
                        // TODO: print out the file has been saved.
                        Console.WriteLine("File saved to {0}", outputFile);
                    }
                }
                else
                {
        ...
        public static List<Weapon> Parse(string fileName)
        {
            ...
            using (StreamReader reader = new StreamReader(fileName))
            {
                ...
                while (reader.Peek() > 0)
                {
                    ...
                    if (values.Length < 4)
                    {
                        Console.WriteLine("Skipping invalid line (expected 4 columns): {0}", line);
                        continue;
                    }
                    ...
                    if (!int.TryParse(values[2].Trim(), out int parsedRarity))
                    {
                        Console.WriteLine("Warning: could not parse Rarity for line: {0}. Setting to 0.", line);
                        parsedRarity = 0;
                    }
                    weapon.Rarity = parsedRarity;

                    if (!int.TryParse(values[3].Trim(), out int parsedBaseAttack))
                    {
                        Console.WriteLine("Warning: could not parse BaseAttack for line: {0}. Setting to 0.", line);
                        parsedBaseAttack = 0;
                    }
                    weapon.BaseAttack = parsedBaseAttack;
